...
def save_to_file(name, elements, status, current_section):
    ...
    try:
        ...
        # dealing with Photos
            ...
            results = []
            # x.get_attribute('href') for x in elements
            for x in range(min(len(elements),3)):
                results.append(elements[x].get_attribute('href'))
                print(results[x])
            results.pop(0)
...
def scrap_profile(ids):
    folder = "/Users/anshu/Documents/sih/facebook/Data"
    ...
    try:
        ...
        try:
            ...
            if platform_ in ['linux', 'darwin']:
                ...
                # driver = webdriver.Firefox(options=options,executable_path="/home/ash/.local/bin/geckodriver")
            else:
...
def main():
    #input = open("/Users/anshu/Documents/sih/facebook/facebook_response.txt", newline='\n')
    #content = input.readlines()
    #usernames = [id.strip() for id in content]
    #ids = [url[25:] for url in usernames]
    #input.close()

    ids = ["https://www.facebook.com/" + line.split("/")[-1] for line in open("/Users/anshu/Documents/sih/facebook/facebook_response.txt", newline='\n')]
    ...
    if len(ids) > 0:
        ...
        login('tarunagarwal27.99@gmail.com','GAsoftware')
        scrap_profile(ids)
        driver.close()
        # else:
        #     print("Username & Password is mandatory. Exiting.....")
        #     exit()
    else:
